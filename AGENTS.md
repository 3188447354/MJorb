...
- **有外层重试的地方，内层预算必须短**（2026-09-20 真机，构建 184）。就绪探测
  ...
  「约 18 秒」放大成 **约 9 分钟**，而且**设备不可达时每轮都走满**（最坏路径 = 最常见路径 ✗）。
  ⚠️ 上限是 9 分钟而不是 18 分钟：`readyDeviceIdentifier()` 第一行是
  ...
  **算预算时取「外层超时」与「被包住那层的内部轮询预算」的较大者** ——
  ...
  ⚠️ 同时要**逐行读控制流、确认哪些调用真的会到达** ——「可能很贵的调用」≠「一定会被调用」
  （这个数字我改了三轮：3.5 分钟 → 9–18 分钟 → ~9 分钟 / ~20–60 秒 ✗✗✓）。
