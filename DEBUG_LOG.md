...
**⚠️ 真机另暴露一处（与签名器无关，待单独查）**：
描述文件回收多轮 `回收中止：阳性对照未通过（com.mjorb.seal.CT8QZ7352B 被答成未安装）` ✗ ——
阳性对照挑的是**旧 Team 后缀的 Seal 自己**，而那个 ID 已被自替换覆盖 ⇒ **必然**答「未安装」✓；
同一行里 R44 的判别诊断显示 `com.apple.Preferences=unavailable`、`com.apple.mobilesafari=unavailable`
