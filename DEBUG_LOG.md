...
- **涉及文件**：`MinimuxerInstallChannel.swift`、删除 `SelfReplacementController.swift`、`AppDetailView.swift`、`verify-certificate-handoff.py`、版本与发布记录。
...
- **修复**：先完成磁盘容量检查、IPA 解包和 Bundle 结构预处理，确认本地已具备可签产物后才允许轮换证书。免费团队没有可覆盖新 7 天周期的本机身份时，按“列举 → 排序 → 撤销不可用证书 → 立即持久化失效状态 → 立即创建并保存 P12”执行；付费团队以明确 3022 为轮换触发。当前 Seal 证书排最后，轮换后自动重签同账号受影响应用并最后安装 Seal。新证书必须通过寿命校验；主应用和所有扩展的 profile 必须匹配本轮申请时间、团队、Bundle ID、UDID、证书和近完整 7 天窗口，旧结果在同一会话重取一次。自续签用后台任务覆盖证书、profile、签名和安装全链路。
- **日志**：记录远端证书总数、每张复用结论、轮换原因与顺序、撤销落盘、新证书有效期/P12 状态、每个 profile 的 UUID/创建/到期/绑定、同一安装通道衔接和受影响应用恢复结果；不记录 Apple ID 与凭据正文。
- **涉及文件**：`ApplePortalSigningService.swift`、`SigningCoordinator.swift`、`SigningCertificateMaterialPolicy.swift`、`ProvisioningProfileBinding.swift`、对应测试和两份静态守护脚本。
- **验证状态**：本地静态守护与发布安全变异检查通过；Swift 编译、单测和 UI 回归交由本次分支 GitHub Actions 验证，真机需安装 1.1.12 后核对导出日志。
...
### 34. 真机反馈：C/E 已通过；证书页缺少「关联 App」可读性
- **真机已确认**：用户在 1.1.9 覆盖安装后验证了「导入中途强杀不误删文件」与「安装失败后到期日保持旧值」—— C/E 两个关键回归通过。
- **新问题**：证书列表里的 machineName 被截断成 `Apple Developmen...hone-...`，用户无法判断这张证书对应哪个 App，也不知道撤销会影响谁。
- **修复**：
  - 新增 `CertificateRevocationImpact.associatedApps`：同时检查 AppRecord 顶层证书序列号与每个 `signingTargets`（含扩展 target）的证书序列号，并做前导 0 归一化。
  - 证书行展示完整 machineName（允许两行）和「证书标识」；新增「关联 App（N 个）」列表，显示 App 名、Bundle ID、当前状态；最多列 5 个，超出显示剩余数量。
  - 无法从本机记录关联时明确显示「本机记录中未找到关联 App」，而不是让用户猜。
- **安全意义**：撤销前用户能看到影响范围；这是对现有「撤销前二次确认」的可视化补强，不自动替用户做决定。
- **验证状态**：新增扩展 target 关联单测；护栏 71 检查 + 38 变异 PASS，Swift 编译与 CI 待跑。

### 35. 远端证书仍在 ≠ 本机可以复用：签名必须有证书对应的私钥
- **真机现象**：用户撤销一张未在本机 App 使用的证书后，账号下还剩一张与 StikDebug 等已安装 App 关联的证书；再次签名仍提示「证书数量已达上限」。用户观察到签名/续签流程都有「准备证书/申请证书」步骤。
- **根因**：Apple 服务器上的证书只有公钥/证书记录；创建证书时的**私钥由生成请求的本机生成，只随响应返回**，Apple 不会把私钥下载回来。当前截图中「Apple 开发证书：无效」「Apple 侧可用于本机：0 个 App ID」说明这台设备的 P12/私钥不可用或不匹配。已安装的 StikDebug 仍能运行，只能说明它包内的 profile/签名仍有效，不能证明当前设备还拥有重新签名所需的私钥。
- **旧行为**：本地 P12 缺失/损坏时，`signingIdentity` 找不到可用身份，直接走 `addCertificate` 申请新证书；远端已有证书时 Apple 返回上限，用户看到的是「证书达到上限」，而不是真正原因「本机缺私钥」。
- **修复**：远端仍存在已保存 serial 对应证书、但本机 P12 无法解析时，停止申请新证书，改报 `SEAL-CERT-204c`「本机缺少证书私钥」，说明已安装 App 可继续运行但新签名不能只靠服务器证书完成，并引导从原设备/备份恢复 P12，或先在「我的」→「签名证书」确认关联 App 后再撤销。
- **UI 补强**：证书页增加「本机签名私钥：可用/缺失或损坏（不能用于新签名）」一行，避免只显示泛化的「无效」。
- **验证状态**：新增 1 个错误语义测试；护栏 71+38 PASS，Swift 编译与 CI 待跑。

### 36. 账号绑定序列号过期后，证书页显示的是「撤销的旧证书」，不是 Apple 关联错证书
- **真机现象**：用户撤销了无关联 App 的证书（有效期 2027-09-13 13:06），但证书页顶部「Apple 开发证书」仍显示这张日期并标「无效」；账号证书列表剩下的另一张（2027-09-13 16:01）才关联 LiveContainer/StikDebug。
- **根因**：`AppleAccountRecord.certificateSerialNumber` 仍绑定撤销的旧 serial；证书页按这个本机绑定 serial 查健康状态，并在 Apple 找不到远端证书时回退显示本机旧 P12 的 `notAfter`，于是出现「无效 + 未来日期」的误导。底部账号证书列表显示的是 Apple 远端库存，两者不是同一个数据源，也不是 Apple 把证书关联错了。
- **签名侧旧行为**：远端找不到本机绑定 serial 时，直接进入 `addCertificate`；如果账号还有另一张证书，就撞上限，错误看起来像「证书数量问题」。但另一张证书的私钥也未必在本机，不能直接拿来用。
- **修复**：
  - `signingIdentity` 发现本机绑定 serial 已不在 Apple 远端列表时，报 `SEAL-CERT-204d`「本机绑定的证书已不存在」，停止盲目申请新证书；
  - 远端证书仍在但本机 P12 缺失时继续报 `SEAL-CERT-204c`「本机缺少证书私钥」；
  - 证书健康状态在 Apple 明确找不到 serial 时不再显示旧 P12 的未来日期，改显示「已撤销或不存在」；
  - 当前剩余证书关联 App 的展示保持不变。
- **安全边界**：不自动把本机账号绑定改成另一张远端证书，因为那张证书的私钥可能不在本机；不自动撤销剩余证书。
- **验证状态**：新增 stale binding 错误语义测试；护栏 73 检查 + 40 变异 PASS，Swift 编译与 CI 待跑。

### 37. 根治证书绑定问题：增加 P12 恢复入口，而不是只改错误文案
- **用户要求**：不能只把「证书达到上限」改成「私钥缺失」；必须能在不撤销已安装 App 所用证书的情况下恢复签名能力。
- **根因约束**：Apple 不下发私钥，远端证书本身无法用于本机签名；唯一根治方式是导入创建该证书时导出的 P12（证书 + 私钥）。
- **修复**：
  - 「我的 → 签名证书」新增「从 P12 备份恢复本机私钥」入口；
  - Seal 解析 P12，确认包含私钥；
  - 将 P12 序列号与当前 Apple ID/Team 的远端证书清单匹配，不允许把其他账号/Team 的 P12 写入当前账号；
  - 匹配成功后保存 P12、序列号、machineIdentifier，并更新本机账号绑定；
  - 恢复完成后重新计算证书健康状态，签名/续签将复用这张远端证书，不再申请新证书，也不需要撤销 LiveContainer/StikDebug 使用的证书。
- **安全边界**：P12 不匹配、没有私钥、账号凭据不可读都拒绝写入；不自动切换远端证书，不自动撤销。
- **验证状态**：护栏 75 检查 + 42 变异 PASS；Swift 编译与 CI 待跑。
