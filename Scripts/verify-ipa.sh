...
if grep -q '^Payload/Seal.app/PlugIns/' <<<"$entries"; then
  echo "Unexpected app extension found in Seal IPA (no-extension build expected)" >&2
  exit 1
fi
...
echo "Unsigned IPA verification passed (no embedded extension)."