...
def violations(load=read):
    ...
    prepare_count = portal_source.count("let prepared = try signingWorkspace.prepare(")
    progress_count = portal_source.count("await progress(.preparingBundle)")
    check(prepare_count > 0 and progress_count == prepare_count,
          "R32: 每一条 `SigningWorkspace.prepare` 路径都必须先发 `.preparingBundle` —— "
          "否则大 IPA 解压会被误显示成「正在验证 Apple ID」")
    ...
    #
# 真机：「阳性对照未通过（com.example.seal.<TEAM_ID> 被答成未安装）」✗ ——
    ...
    #
    # 现在收敛成路径感知的预算表 + 一组纯函数：完整重签有十个阶段，profile-only
    # 只会走其中六个真实阶段。两条路径要分别守：
    #   ① 路径上每个阶段都必须有预算 —— 漏一个，界面上那个阶段会停在上一阶段的数值上；
    #   ② 每张表内部的连续性 `ceiling_i == floor_{i+1}` —— 破了它，阶段切换时进度会跳一下或往回退。
    ...
    full_budget_source, profile_budget_source = budget_source.split(
        "private static func profileOnlyPlan(for stage: SigningStage) -> Plan {", 1
    )
    profile_budget_source = profile_budget_source.split("static func bucketTotal(", 1)[0]
    for stage_name in stage_names:
        check("case .%s:" % stage_name in full_budget_source,
              "R36: 阶段 `%s` 没有进度预算 —— 界面上它会停在上一阶段的数值上" % stage_name)
    budget_rows = re.findall(r"floor: ([0-9.]+), ceiling: ([0-9.]+),", full_budget_source)
    ...
    for row_index in range(len(budget_rows) - 1):
        check(budget_rows[row_index][1] == budget_rows[row_index + 1][0],
              ...
              "破了它，阶段切换时进度会跳一下或往回退")
    profile_stage_names = (
        "waitingForChannel", "preparingAccount", "preparingBundle",
        "preparingCertificate", "preparingAppID", "preparingProfiles",
    )
    for stage_name in profile_stage_names:
        check("case .%s:" % stage_name in profile_budget_source,
              "R36: profile-only 阶段 `%s` 没有进度预算" % stage_name)
    profile_rows = re.findall(r"floor: ([0-9.]+), ceiling: ([0-9.]+),", profile_budget_source)
    check(len(profile_rows) == len(profile_stage_names),
          "R36: profile-only 预算表行数（%d）与真实阶段数（%d）不一致" % (
              len(profile_rows), len(profile_stage_names)))
    for row_index in range(len(profile_rows) - 1):
        check(profile_rows[row_index][1] == profile_rows[row_index + 1][0],
              "R36: profile-only 阶段预算必须首尾相接（ceiling_i == floor_{i+1}）")
    ...
    check("func everyStageBudgetMeetsTheNextOne()" in budget_tests
          ...
          "R36: 单测必须断言「阶段预算首尾相接」")
    check("func profileOnlyStageBudgetsMeetTheNextOne()" in budget_tests
          and "renewalExecutionPath: .profileOnly" in budget_tests,
          "R36: 单测必须断言「profile-only 阶段预算首尾相接」")
...
def main():
    ...
    mutations += [
        ...
        ("Seal/Core/Signing/SigningProgressBudget.swift",
         ...
         "floor: 14, ceiling: 30,",
         "R36: 阶段预算必须首尾相接"),
        # profile-only 的 App ID 阶段若不再接到描述文件阶段，五格轨道会在切换处跳动。
        ("Seal/Core/Signing/SigningProgressBudget.swift",
         "floor: 55, ceiling: 75,",
         "floor: 55, ceiling: 70,",
         "R36: profile-only 阶段预算必须首尾相接"),
        ...
        ("Seal/Infrastructure/Signing/ApplePortalSigningService.swift",
         ...
         "            let prepareStartedAt = Date()",
         "R32: 每一条 `SigningWorkspace.prepare` 路径都必须先发"),
