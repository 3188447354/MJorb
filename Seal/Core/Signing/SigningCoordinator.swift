...
actor SigningCoordinator {
    ...
    func signAndInstall(
        ...
        // 已安装应用的续签路径只有在当前账号、设备与签名身份均完成核验后才能确定。
        // UI 据此区分「仅更新描述文件」与「完整重签并安装」，不能从用户入口猜测。
        onRenewalExecutionPath: @Sendable @escaping (RenewalExecutionPath) async -> Void = { _ in },
        ...
        do {
            ...
            if useProfileOnlyRenewal {
                ...
                return try await renewProfilesOnly(
                    ...
                    onCertificateResolved: onCertificateResolved
                )
            }
