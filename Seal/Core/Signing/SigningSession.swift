...
/// A renewal does not necessarily install a newly signed IPA. The coordinator
/// resolves this only after it has checked the current account, device, and
/// installed signing identity, so the presentation must treat it as unknown
/// until then.
enum RenewalExecutionPath: Equatable, Sendable {
    case profileOnly
    case fullResign

    var successTitle: String {
        switch self {
        case .profileOnly: "描述文件续签完成"
        case .fullResign: "续签并安装成功"
        }
    }

    var confirmedLogMessage: String {
        switch self {
        case .profileOnly:
            "续签路径已确认：仅更新描述文件；不会重新签名、打包或安装 IPA。"
        case .fullResign:
            "续签路径已确认：需要完整重签并安装。"
        }
    }

    func stageTitle(for stage: SigningStage) -> String {
        switch self {
        case .fullResign:
        ...
        case .profileOnly:
            ...
            case .preparingAppID: return "正在核对 App ID"
            case .preparingProfiles: return "正在申请描述文件"
            case .signing, .pushing, .installing, .verifying: return "正在更新描述文件"
            }
        }
    }
}
...
struct SigningSession: Identifiable, Equatable, Sendable {
    ...
    /// Nil until `SigningCoordinator` has verified which renewal path is safe.
    var renewalExecutionPath: RenewalExecutionPath?
    var status: Status
    ...
    init(
        ...
        self.renewalExecutionPath = renewalExecutionPath
