...
final class AppsViewModel: ObservableObject {
    ...
    private func consumeBatchEvent(_ event: BatchRefreshEvent) {
        ...
        case .appRenewalExecutionPath(let index, let total, let app, let path):
            batchRefreshSession?.currentIndex = index
            batchRefreshSession?.total = total
            batchRefreshSession?.currentAppName = app.displayName
    ...
    private func startSigning(
        ...
        if app.belongsInInstalledList {
            startMessage = "开始续签：\(app.name)，Apple ID：\(account.maskedEmail)，Team：\(account.teamID)，Bundle ID：\(targetBundleIdentifier)"
        } else {
            startMessage = "准备签名：\(app.name)，Apple ID：\(account.maskedEmail)，Team：\(account.teamID)，Bundle ID：\(targetBundleIdentifier)"
        }
        Task { [weak self] in
            try? await self?.logStore?.append(
    ...
    private func updateRenewalExecutionPath(_ path: RenewalExecutionPath) async {
        guard signingSession?.app.belongsInInstalledList == true else { return }
        signingSession?.renewalExecutionPath = path
        try? await logStore?.append(category: .renewal, message: path.confirmedLogMessage)
    }
