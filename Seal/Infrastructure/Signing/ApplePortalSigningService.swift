...
actor ApplePortalSigningService {
    ...
    func sign(
        ...
        do {
            ...
            //
            // 覆盖的后果（用户 2026-09-18 实测原话：「重新添加 318***5*** 后我去签另一个应用
    ...
    /// Requests fresh provisioning profiles for an already installed app without
    /// re-signing its IPA. This path deliberately refuses to create, rotate, or
    /// revoke a certificate: a profile-only refresh can only preserve the exact
    /// signing identity that is already installed on the device.
    func prepareProfileOnlyRenewal(
        ...
        guard let team = teams.first(where: { $0.identifier == account.teamID }) else {
            throw Self.failure(
                title: "Team 不匹配",
                reason: "Apple 返回的团队列表中已找不到已保存的 Team ID；无法安全续签现有应用。",
                recovery: "选择正确的 Apple ID 或重新完整签名",
                code: "SEAL-PROFILE-330"
            )
        }
        ...
        let resolvedSerial = try await existingProfileOnlyCertificate(
            secret: secret,
            expectedSerialNumber: certificateSerialNumber,
            team: team,
            session: session
        )
        ...
        guard preparation.droppedExtensionBundleIdentifiers.isEmpty else {
            throw Self.failure(
                title: "扩展身份不完整",
                reason: "profile-only 续签不允许移除扩展；所有已安装目标必须取得新描述文件。",
                recovery: "执行完整重签并确认扩展配置",
                code: "SEAL-PROFILE-332"
            )
        }

        var materials: [ProfileOnlyProfileMaterial] = []
        for profile in preparation.profiles {
            let binding = try ProvisioningProfileReader().binding(from: profile.data).validated(
                expectedTeamID: team.identifier,
                expectedBundleID: profile.bundleIdentifier,
                expectedCertificateSerialNumber: resolvedSerial,
                expectedDeviceIdentifier: deviceIdentifier,
                requestedAfter: requestedAt.addingTimeInterval(-Self.profileRequestClockTolerance),
                minimumRemainingLifetime: Self.minimumFreshProfileLifetime
            )
            guard binding.profileUUID?.isEmpty == false else {
                throw Self.failure(
                    title: "描述文件身份缺失",
                    reason: "Apple 返回的 \(binding.bundleIdentifier) 描述文件没有 UUID，无法在设备端精确读回核验。",
                    recovery: "执行完整重签以重新建立应用身份",
                    code: "SEAL-PROFILE-336"
                )
            }
            try ProvisioningProfileBinding.validateEntitlements(
                requested: preparation.requestedEntitlements[binding.bundleIdentifier] ?? [:],
                profile: binding.entitlements,
                bundleIdentifier: binding.bundleIdentifier
            )
            materials.append(ProfileOnlyProfileMaterial(data: profile.data, binding: binding))
        }
    ...
    private func existingProfileOnlyCertificate(
        ...
        ), Self.certificateReusable(local) else {
            throw Self.failure(
                title: "当前证书不可续签",
                reason: "profile-only 只能复用设备上当前应用使用、且仍能覆盖完整 7 天描述文件寿命的本机证书。",
                recovery: "执行完整重签以申请新的签名证书",
                code: "SEAL-PROFILE-334"
            )
        }
        let certificates = try await withSessionRecovery("核验当前签名证书", retriesOnTimeout: true) {
        ...
        guard certificates.contains(where: {
            SigningCertificateSelectionPolicy.normalizedSerialNumber($0.serialNumber)
                == SigningCertificateSelectionPolicy.normalizedSerialNumber(expectedSerialNumber)
        }) else {
            throw Self.failure(
                title: "当前证书已不可用",
                reason: "Apple 证书列表中已没有设备上当前应用使用的证书；继续注入 profile 无法恢复应用。",
                recovery: "执行完整重签以建立新的签名身份",
                code: "SEAL-PROFILE-335"
            )
        }
        return local.serialNumber
    }
    ...
    private func provisioningProfiles(
        ...
        allowDroppingExtensions: Bool,
        requiresExistingAppIDs: Bool = false,
        team: ALTTeam,
        session: ALTAppleAPISession,
        progress: @escaping @Sendable (SigningStage) async -> Void,
        onWorkUnits: @escaping @Sendable (SigningWorkUnits) async -> Void = { _ in }
        ...
        for (originalBundleID, mappedBundleID) in ApplePortalAppIDResolver.preparationOrder(
            ...
            do {
                ...
                throw CancellationError()
            } catch let failure as ImportFailure where requiresExistingAppIDs {
                throw failure
            } catch {
                guard mappedBundleID != mappedMainBundleID else { throw error }
...
struct ProfileOnlyPortalResult: Sendable {
