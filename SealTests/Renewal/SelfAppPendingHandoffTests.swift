...
struct SelfAppPendingHandoffTests {
    ...
    @Test
    func startupReconciliationLogsEveryDecisiveOutcome() async throws {
        let outcomes: [(SelfReplacementReconcileAction, String)] = [
            (.settle, "自替换结算确认："),
            (.closeAsNotInstalled, "自替换结算：仍在安装前身份"),
            (.requireRecovery(reason: "fixture identity mismatch"), "自替换结算：需要恢复")
        ]

        for (outcome, expectedPrefix) in outcomes {
            let logDirectory = FileManager.default.temporaryDirectory
                .appending(path: "SealReplacementOutcome-\(UUID().uuidString)", directoryHint: .isDirectory)
            defer { try? FileManager.default.removeItem(at: logDirectory) }
            let logStore = SealLogStore(
                fileURL: logDirectory.appending(path: "Logs.json"),
                fileProtector: MarkerFileProtector()
            )
            let fixture = try await ReplacementRegistrarFixture.make(action: outcome, logStore: logStore)
            defer { try? FileManager.default.removeItem(at: fixture.root) }

            try await fixture.registrar.ensureRegistered()

            let entries = try await logStore.entries()
            let logged = entries.contains { $0.message.hasPrefix(expectedPrefix) }
            #expect(logged, "\(expectedPrefix) 必须写进日志，才能从导出的证据判断 Seal 自替换的真实终态")
        }
    }
