...
struct SigningProgressBudgetTests {
    ...
    @Test
    func profileOnlyStageBudgetsMeetTheNextOne() {
        let stages: [SigningStage] = [
            .waitingForChannel,
            .preparingAccount,
            .preparingBundle,
        ...
        for index in 0..<(stages.count - 1) {
            let current = SigningProgressBudget.plan(
                for: stages[index],
                renewalExecutionPath: .profileOnly
            )
            let next = SigningProgressBudget.plan(
                for: stages[index + 1],
                renewalExecutionPath: .profileOnly
            )
            #expect(current.ceiling == next.floor)
