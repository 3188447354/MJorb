...
struct SigningWorkUnitsProgressTests {
    ...
    @Test
    func profileOnlyTrackUsesAllFiveCellsWithoutInventingInstallStages() {
        let path: RenewalExecutionPath? = .profileOnly
        let realStages: [SigningStage] = [
            .waitingForChannel,
            .preparingAccount,
            .preparingBundle,
        ...
        let buckets = Set(realStages.map {
            SigningProgressBudget.plan(for: $0, renewalExecutionPath: path).bucket
        })
        #expect(buckets == Set(0..<SigningProgressBudget.bucketCount))
        #expect(
            SigningProgressBudget.plan(
                for: .preparingProfiles,
                renewalExecutionPath: path
            ).bucket == 4
        )

        let firstProfile = SigningProgressBudget.bucketFill(
            4,
            stage: .preparingProfiles,
            elapsed: 0,
            realProgress: nil,
            workUnits: SigningWorkUnits(stage: .preparingProfiles, done: 1, total: 4),
            renewalExecutionPath: path
        )
        #expect(firstProfile == 0.25)
        #expect(
            SigningProgressBudget.bucketFill(
                3,
                stage: .preparingProfiles,
                elapsed: 0,
                realProgress: nil,
