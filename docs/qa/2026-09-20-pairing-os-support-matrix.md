...
| 设备系统 | 远程配对 | 具体版本 |
|---|---|---|
| iOS / iPadOS 17.0 – 17.3.1 | ❌ | 17.0、17.0.1、17.0.2、17.0.3、17.1、17.1.1、17.1.2、17.2、17.2.1、17.3、17.3.1（🔴 **2026-09-24 起 Seal 自身也装不上**，见第五节） |
...
> 🔴 **2026-09-24 起：Seal 最低支持 iOS 17.4**（用户指令「16.0-17.3.1 禁止使用」）。
> ⇒ **iOS 16.0–17.3.1 一律装不上 Seal**，与配对方式无关。下表「远程配对」一列仍是
> **配对助手**的能力（不是 Seal 的门槛）—— 助手对 17.0–17.3.1 的 Lockdown 分流**代码保留**，
> 老版本 Seal（≤ 1.3.7）的用户仍可用。
...
1. **Seal 自己的部署目标是 iOS 17.4**（2026-09-24 起；此前是 17.0）。
   部署目标共 **9 处**必须同步：`project.yml` 5 处（`options.deploymentTarget.iOS` + 4 个 target）、
   `Config/Base.xcconfig` 1 处、三份 workflow 各 1 处硬编码断言 —— 守卫 **R69** 已钉住。
   ⇒ **iOS 17.3.1 及以下无法安装 Seal**，与配对方式无关。
2. **iOS 17.0–17.3.1**（历史结论，仅对 **≤ 1.3.7** 的 Seal 成立）：只能走 Lockdown（本机配对）。
   minimuxer 里这条链路是完整的
   ...
   **配对助手自 run#185 起会按设备版本自动选**（`seal_mode_for_ios`）。
   ✅ 这条链路**已于 2026-09-24 在 iOS 17.0–17.3.1 真机跑通**（构建 190：配对 → 导入 → 验证 →
   装 App → 点开 → 续签含批量，全通；回归清单第 15 项结案）。
   ⚠️ 它**必须开 LocalDevVPN**（设备 IP 来自 `utun` 对端）。
...
| # | 查了什么 | 结果 |
|---|---|---|
| 1 | `Seal/**` 与 `Minimuxer/**` 里有没有 `17.4` **运行期**闸门 | **一处都没有** ✓ —— 全仓唯一的运行期 17.4 判据就在配对助手（本轮已修）。<br>🔴 **2026-09-24 补充**：门槛改由**部署目标**承担（iOS 17.4，共 9 处，守卫 R69 钉住），源码里仍**没有**运行期版本分支 |
